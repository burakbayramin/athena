...
plan: 04
subsystem: cli
tags: [dry-run, plan-cache, assigned-agents, serde, planner]

# Dependency graph
requires:
  - phase: 08-cli-and-progress plan 03
    provides: "CLI run wiring, progress observer seam, and verbose transcript rendering"
provides:
  - "Project-local routed plan cache at `.ath/last-plan.json` for cost-free dry-runs"
  - "Early dry-run branch that exits before config, provider, or execution setup"
  - "Assigned-agent-aware execution plan display reused by both live runs and cached dry-runs"
affects: [08-cli-and-progress, 09-reporting-and-error-quality]

# Tech tracking
tech-stack:
  added: [serde_json, tempfile]
  patterns: [local-plan-cache, early-dry-run-branch, shared-plan-renderer]
...
key-decisions:
  - "Dry-run reads `.ath/last-plan.json` from the current project root instead of attempting to re-plan without cost"
  - "The dry-run branch returns before `ConfigStore::load()` so missing providers never block cached previews"
  - "Assigned agents are rendered through the existing plan formatter so cached previews and live plan output stay aligned"

patterns-established:
  - "Normal runs persist the routed, isolation-checked `ExecutionPlan` before execution so future dry-runs are honest"
  - "Cached plan previews reuse the planner display path instead of introducing a second formatter with drift risk"
