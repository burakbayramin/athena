...
plan: 02
subsystem: input
tags: [llm-parsing, json-schema, structured-output, genai, retry-logic, spec-file]

# Dependency graph
requires:
  - phase: 04-input-parsing
    provides: "InputMode enum, InputError type, AgentRequest.json_schema field"
  - phase: 02-agent-clients
    provides: "AgentBackend trait, MockBackend, call_provider, circuit breaker"
provides:
  - "parse_to_project_spec function with retry-on-error logic"
  - "LLM system prompts for natural language, spec file, and codebase modes"
  - "AgentRequest builders with json_schema for structured output"
  - "project_spec_json_schema() for JsonSpec enforcement"
  - "read_spec_file with 50KB size cap"
  - "call_provider json_schema threading to genai ChatOptions::JsonSpec"
affects: [04-input-parsing, 05-phase-planner]

# Tech tracking
tech-stack:
  added: []
  patterns: [retry-with-error-feedback, request-builder-closure, json-schema-structured-output]
...
key-decisions:
  - "request_builder closure pattern allows all 3 input modes to share parse_to_project_spec retry logic"
  - "json_schema threaded through run_with_retry_and_breaker to call_provider, enabling per-request structured output"

patterns-established:
  - "Request builder closure: parse_to_project_spec takes Fn(Option<&str>) -> AgentRequest for mode-agnostic retry"
  - "Retry with error feedback: append last_error to prompt on each retry attempt"
  - "Size-before-read: check metadata().len() before tokio::fs::read_to_string"
