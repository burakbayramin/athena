...
plan: 03
subsystem: input
tags: [codebase-scanner, gitignore, ignore-crate, key-file-extraction, cli-wiring, project-spec]

# Dependency graph
requires:
  - phase: 04-input-parsing
    provides: "InputMode enum, InputError type, parse_to_project_spec, prompt builders"
  - phase: 02-agent-clients
    provides: "AgentBackend trait, MockBackend"
provides:
  - "scan_codebase with gitignore-aware traversal and key file extraction"
  - "is_key_file heuristic for manifest/README/entry point detection"
  - "parse_input high-level function dispatching all three InputMode variants"
  - "display_project_spec_summary for terminal-formatted ProjectSpec output"
  - "CLI wired to resolve input mode from flags"
affects: [05-phase-planner, 07-orchestrator]

# Tech tracking
tech-stack:
  added: []
  patterns: [ignore-crate-walkbuilder, spawn-blocking-for-sync-io, per-file-and-total-size-caps]
...
key-decisions:
  - "scan_codebase is sync (std::fs), called from async context via tokio::task::spawn_blocking"
  - "Key file detection uses filename matching + relative path patterns for entry points"

patterns-established:
  - "Size-capped extraction: per-file (5KB) and total (30KB) caps prevent token explosion"
  - "WalkBuilder with filter_entry callback for skip-dir logic"
