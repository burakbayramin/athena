...
plan: 01
subsystem: planner
tags: [dag, topological-sort, graph-algorithms, serde, thiserror]

# Dependency graph
requires:
  - phase: 01-foundation
    provides: "ath-types crate with ValidationError, SkillTag, ProjectSpec"
  - phase: 02-agent-clients
    provides: "AgentError type for DecomposeError wrapping"
provides:
  - "ExecutionPlan, PhaseSpec, TaskSpec, ContractLabel types in ath-types"
  - "topological_sort, compute_parallel_groups, critical_path_length algorithms"
  - "5 new ValidationError DAG variants (CircularDependency, MissingDependencyTarget, OrphanedGoal, EmptyPhase, UnsatisfiedContract)"
  - "DecomposeError enum in ath-planner::decompose"
affects: [05-phase-decomposition, 06-module-isolation, 07-review-engine, 08-cli-progress]

# Tech tracking
tech-stack:
  added: []
  patterns: [kahn-topological-sort, dfs-cycle-detection, level-based-parallel-groups]
...
key-decisions:
  - "Kahn's algorithm for topological sort with BFS queue and deterministic ordering via sorted initial queue"
  - "DFS color marking (White/Gray/Black) for cycle detection -- backtrack from Gray revisit to extract cycle path"
  - "Level assignment for parallel groups: level = max(dep levels) + 1, group by level"
  - "ContractLabel as type alias (String) rather than newtype -- flexibility for LLM-generated labels"

patterns-established:
  - "DAG algorithms as pure functions on &[PhaseSpec] -- no I/O, fully deterministic, testable without mocks"
  - "Helper make_phase() test factory for minimal PhaseSpec construction in tests"
