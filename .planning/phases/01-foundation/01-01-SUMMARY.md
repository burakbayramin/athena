...
plan: 01
subsystem: infra
tags: [cargo, workspace, rust, cli, clap]

# Dependency graph
requires: []
provides:
  - Cargo workspace scaffold with 7 ath-* crates
  - Workspace-level dependency management via [workspace.dependencies]
  - ath CLI binary responding to --version and --help
  - Unidirectional dependency graph (ath-types at root)
affects: [01-02, 01-03, 01-04, all-future-phases]

# Tech tracking
tech-stack:
  added: [serde 1.0, serde_json 1.0, thiserror 2.0, anyhow 1.0, clap 4.5, toml 0.8, dotenvy 0.15, chrono 0.4, uuid 1.8, dirs 6, colored 3]
  patterns: [cargo-workspace, workspace-dependencies, path-dependencies]
...
key-decisions:
  - "Used workspace.package for version/edition inheritance across all crates"
  - "Internal crates listed in [workspace.dependencies] for consistent path references"

patterns-established:
  - "Workspace layout: all crates under crates/ with ath-* prefix"
  - "Dependency direction: ath-types has zero ath-* deps, all others depend on it"
  - "CLI binary named 'ath' via [[bin]] in ath-cli"
